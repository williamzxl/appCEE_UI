...
def get_all_trans_answer(list, taskID=1):
    url = "http://192.168.1.154:55262/sysListening/{}/wordTrans".format(list)
...
def right_answer_trans(answer, num):
    get_answer = answer[:]
    right_answer = get_answer.pop(int(num)-1)
    return right_answer


def wrong_answer_trans(answer, num):
    get_answer = answer[:]
