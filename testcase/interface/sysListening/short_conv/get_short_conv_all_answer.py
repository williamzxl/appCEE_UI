...
def get_all_sen_fill_answer(list, taskID=1):
    url = "http://192.168.1.154:55262/sysListening/{}/senFill".format(list)
...
def right_answer_sen_fill(answer, num):
    get_answer = answer[:]
    right_answer = get_answer.pop(int(num)-1)
    return right_answer


def wrong_answer_sen_fill(answer, num):
    get_answer = answer[:]
